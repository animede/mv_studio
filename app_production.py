...
def _list_generated_movie_items(client_session_id: Optional[str], *, limit: int = 24) -> List[Dict[str, Any]]:
    ...
    for root in search_roots:
        ...
        for path in candidates:
            ...
            items.append(_build_mv_library_item(path, client_session_id, library_index.get(media_key)))
...
def production_final_mv_list(client_session_id: Optional[str] = None, limit: int = 24):
    items = _list_generated_movie_items(client_session_id, limit=limit)
    return {
        "success": True,
        "items": items,
        "count": len(items),
...
def production_final_mv_library_metadata(request: ProductionMVLibraryMetadataRequest):
    path = _resolve_preview_media_source(request.filename, request.client_session_id)
...
def production_final_mv_library_delete(
    ...
    path = _resolve_preview_media_source(filename, client_session_id)
    if _classify_preview_media_type(path) != "video":
