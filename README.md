...
`simple_video_app` は、ComfyUI をバックエンドとして使う standalone アプリです。

現在の既定GUIは **Music Video Studio**（新しいプロダクション版）です。
従来の `Generative Media Place` / `Simple Video` 系UIも互換用として残しています。
...
ブラウザで `http://127.0.0.1:8091/` を開くと、**Music Video Studio** が表示されます。

> 旧GUIを使う場合は、従来どおり `bash start.sh`（または `start.bat`）を使用してください。
...
Windows で旧GUIを使う場合は、`start.bat` が `start.sh` と同等のオプション（`--host`, `--port`, `--comfyui-server` 等）に対応しています。
...
10. ブラウザで `http://127.0.0.1:8091/` を開き、**Music Video Studio** が表示されることを確認
...
```bash
curl -s http://127.0.0.1:8188/system_stats | head
cd simple_video_app && source .venv/bin/activate && python -m uvicorn app_production:app --host 127.0.0.1 --port 8091 --reload
```
...
- `start_production.sh` は `.venv`（`./.venv` または `../.venv`）を自動検出して有効化します
- `start_production.sh` は新しい **Music Video Studio** の起動直後GUIを配信します
- 仮想環境を明示したい場合は `python -m uvicorn app_production:app ...` を使ってください
- 旧GUI / 従来API中心の運用が必要な場合は、下記の `start.sh` / `start_server.sh` を使用してください

## 旧GUI / API起動（互換用）

従来の `Generative Media Place` / `Simple Video` 系UI、および既存の `start.sh` / `start_server.sh` 運用方法です。
...
旧GUI / API を `start.sh` で起動している場合は、互換確認として次も利用できます。
...
- Music Video Studio 変更ドキュメント: [docs/MUSIC_VIDEO_STUDIO_JP.md](docs/MUSIC_VIDEO_STUDIO_JP.md)
...
- `app.py`: 旧GUI / API 用 FastAPI サーバ（静的配信 + API）
- `app_server.py`: マルチユーザー版エントリポイント
- `app_production.py`: 新GUI（Music Video Studio）用 FastAPI サーバ
- `start_production.sh`: 新GUI起動スクリプト
- `start.sh`: standalone 起動スクリプト
- `start_server.sh`: マルチユーザー版起動スクリプト
- `static/index.html`: 旧GUI画面
- `static/music_video_studio.html`: 新GUI画面
- `static/js/music_video_studio.js`: 新GUIロジック
- `static/css/music_video_studio.css`: 新GUIスタイル
