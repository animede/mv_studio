...
`simple_video_app` is a standalone app that uses ComfyUI as the backend.

The current default GUI is **Music Video Studio** (production UI).
The older `Generative Media Place` / `Simple Video` UI is still kept for compatibility.
...
3. Start app:
  - **Linux / macOS**: `bash start_production.sh`
  - **Windows**: `.venv\Scripts\python -m uvicorn app_production:app --host 127.0.0.1 --port 8091 --reload`

Open `http://127.0.0.1:8091/` in your browser to view **Music Video Studio**.

> If you need the older UI, use `bash start.sh` (or `start.bat`) as before.
...
If you need the older UI on Windows, `start.bat` supports the same options as `start.sh` (`--host`, `--port`, `--comfyui-server`, etc.).
...
```bash
curl -I -s http://127.0.0.1:8091/ | head -n 1
curl -s http://127.0.0.1:8091/api/v1/production/config | head
```

9. Check production config:

```bash
curl -s http://127.0.0.1:8091/api/v1/production/config | python3 -m json.tool | head -n 20
```

10. Open `http://127.0.0.1:8091/` and confirm that **Music Video Studio** is displayed.
...
```bash
bash start_production.sh --help
bash start_production.sh --host 0.0.0.0 --port 8091
...
- `start_production.sh` auto-activates `.venv` (`./.venv` or `../.venv`) if found
- `start_production.sh` serves the new **Music Video Studio** startup GUI
- If you want to explicitly use the virtual environment, run `python -m uvicorn app_production:app ...`
- If you still need the older UI / existing API flow, use `start.sh` / `start_server.sh` below

## Legacy UI / API Startup (Compatibility)

This section describes the older `Generative Media Place` / `Simple Video` UI and the existing `start.sh` / `start_server.sh` flow.
...
bash start_server.sh --host 0.0.0.0 --port 8091
...
```bash
SIMPLE_VIDEO_MULTI_USER=1 uvicorn app_server:app --host 0.0.0.0 --port 8091
```
...
```bash
curl -s http://127.0.0.1:8091/ | head
curl -s http://127.0.0.1:8091/api/v1/production/config | head
```

If you are running the legacy UI / API with `start.sh`, you can also use this compatibility check:

```bash
curl -s http://127.0.0.1:8091/api/v1/workflows
```

## Usage Docs

- Music Video Studio change document (JP): [docs/MUSIC_VIDEO_STUDIO_JP.md](docs/MUSIC_VIDEO_STUDIO_JP.md)
...
- `app.py`: FastAPI server for the legacy UI / API flow
- `app_server.py`: Multi-user mode entry point
- `app_production.py`: FastAPI server for the new Music Video Studio GUI
- `start_production.sh`: startup script for the new GUI
- `start.sh`: standalone startup script
- `start_server.sh`: multi-user mode startup script
- `static/index.html`: legacy UI screen
- `static/music_video_studio.html`: new GUI screen
- `static/js/music_video_studio.js`: new GUI logic
- `static/css/music_video_studio.css`: new GUI styles
