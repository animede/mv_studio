(() => {
  ...
  async function refreshGeneratedMvGallery({ rerender = true } = {}) {
    ...
    try {
      const response = await fetch(`/api/v1/production/final-mv/list?client_session_id=${encodeURIComponent(getSessionId())}&limit=24`, {
        cache: 'no-store',
      });
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      const data = await response.json();
      generatedMvGallery.items = Array.isArray(data?.items)
  ...
  function renderGeneratedMvGallery() {
    const toolbarLabel = generatedMvGallery.loadedAt
      ? `最終取得: ${formatDateTime(generatedMvGallery.loadedAt)}`
    ...
    : '<div class="character-output-placeholder">プレビューを表示できません</div>'}
        <div class="generated-mv-card-body">
          ...
          <label class="generated-mv-field">
            <span class="field-label">メモ</span>
            <textarea class="prompt-textarea generated-mv-note-input" data-mv-memo-input rows="3" ${saveBusy ? 'disabled' : ''}>${escapeHtml(item.memo || '')}</textarea>
          </label>
    ...
    return `
      <div class="generated-mv-toolbar">
        <div class="field-help">${escapeHtml(generatedMvGallery.error || toolbarLabel)}</div>
        <button id="generatedMvRefreshBtn" class="detail-action-btn secondary compact" type="button" ${generatedMvGallery.loading ? 'disabled' : ''}>${generatedMvGallery.loading ? '更新中...' : '一覧を更新'}</button>
      </div>
  ...
  function renderFinalMvWorkspace(step, mode) {
    ...
    const generatedMvCount = generatedMvGallery.items.length;
      ...
      <div class="detail-layout">
        <div class="detail-grid">
          <section class="detail-highlight">
            ...
            <ul class="detail-list">
              ...
              <li><strong>保存済みMV</strong>${escapeHtml(generatedMvCount ? `${generatedMvCount}件表示中` : '未取得')}</li>
            </ul>
          </section>
        ...
        <div class="detail-grid">
          ...
          <section class="detail-block">
            ...
            <ul class="detail-list">
              <li><strong>表示件数</strong>${escapeHtml(generatedMvCount ? `${generatedMvCount}件` : '未取得')}</li>
  ...
  function bindEvents() {
    ...
    els.detailBody.addEventListener('click', async (event) => {
      ...
      if (event.target.closest('#generatedMvRefreshBtn')) {
        try {
          await refreshGeneratedMvGallery();
        } catch (error) {
