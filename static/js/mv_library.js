(() => {
  ...
  function renderSummary() {
    ...
    els.summary.innerHTML = `
      <div class="canvas-summary-grid mv-library-summary-cards">
        ...
        <div class="summary-card"><span class="summary-key">表示中</span><strong>${escapeHtml(state.items.length)}</strong></div>
  ...
  function renderList() {
      ...
      return `
        <article class="generated-mv-card" data-mv-library-card data-mv-filename="${escapeHtml(item.filename || '')}" data-mv-subfolder="${escapeHtml(item.subfolder || '')}">
          ...
          <div class="generated-mv-card-body">
            ...
            <label class="generated-mv-field">
              <span class="field-label">メモ</span>
              <textarea class="prompt-textarea generated-mv-note-input" data-mv-memo-input rows="1" ${(saveBusy || deleteBusy) ? 'disabled' : ''}>${escapeHtml(item.memo || '')}</textarea>
            </label>
    ...
    els.listContent.innerHTML = `
      <div class="generated-mv-toolbar">
        <div class="field-help">${escapeHtml(state.error || `最終取得: ${formatDateTime(state.loadedAt)} ・ ${state.items.length}/${state.totalCount || state.items.length}件を表示中`)}</div>
        <div class="generated-mv-links">
          <button id="generatedMvRefreshBtn" class="detail-action-btn secondary compact" type="button" ${state.loading ? 'disabled' : ''}>${state.loadingMode === 'refresh' ? '更新中...' : '一覧を更新'}</button>
          ${state.hasMore ? `<button id="generatedMvLoadMoreBtn" class="detail-action-btn secondary compact" type="button" ${state.loading ? 'disabled' : ''}>${state.loadingMode === 'append' ? '続きを読込中...' : '次を表示'}</button>` : ''}
        </div>
      </div>
  ...
  async function refreshGallery({ append = false } = {}) {
    ...
    state.loading = true;
    state.loadingMode = append ? 'append' : 'refresh';
    if (!append) {
      state.error = '';
    }
    setStatus(append ? '続きを読み込み中...' : '一覧を読み込み中...', 'info');
    renderAll();
    try {
      const response = await fetch(`/api/v1/production/final-mv/list?client_session_id=${encodeURIComponent(getSessionId())}&limit=${encodeURIComponent(state.pageSize)}&offset=${encodeURIComponent(offset)}`, {
        cache: 'no-store',
      });
      if (!response.ok) {
      ...
      const incomingItems = Array.isArray(data?.items) ? data.items.map((item) => normalizeItem(item)).filter(Boolean) : [];
      state.items = append ? mergeItems(state.items, incomingItems) : incomingItems;
      state.loadedAt = Date.now();
      state.totalCount = Math.max(state.items.length, Number(data?.total_count || 0) || 0);
      state.hasMore = !!data?.has_more;
      state.nextOffset = Math.max(state.items.length, Number(data?.next_offset || 0) || 0);
      setStatus(append ? `さらに${incomingItems.length}件を表示しました` : `一覧を更新しました（${state.items.length}/${state.totalCount || state.items.length}件表示）`, 'success');
    } catch (error) {
  ...
  function bindEvents() {
    document.body.addEventListener('click', async (event) => {
      if (event.target.closest('#generatedMvRefreshBtn')) {
        try {
          await refreshGallery({ append: false });
        } catch (error) {
      ...
      if (event.target.closest('#generatedMvLoadMoreBtn')) {
        try {
          await refreshGallery({ append: true });
        } catch (error) {
          setStatus(error.message || '続きを表示できませんでした', 'warning');
        }
        return;
      }
